...
To use this package, *DS9* and *XPA* must be installed on your computer.  If
this is not the case, they are available for different operating systems.  For
example, on Ubuntu, just do:
...
If a single scalar integer or floating point is expected, the following methods
are available:
...
(provided by the `IPC.jl` package), the shared memory segment is directly used
by DS9; otherwise, the contents of the array is sent to DS9.  DS9 will display
the data in the currently selected frame with the current scale parameters,
zoom, orientation, rotation, etc.

To retrieve the array in the current DS9 frame, do:
...
`IP:port` for a TCP/IP socket or it is the path to the socket file for an
AF/Unix socket.  The access point `apt` may be a fully qualified identifier or
a template of the form `class:name` like `"DS9:*"` which correspond to any
instance of the class `"DS9"`.  Note that `name` correspond to the argument of
the `-title` option when DS9 is launched.  See
...
Julia interface to SAOImage/DS9 is built on top of an implementation of an
interface to use the XPA Message System by Eric Mandel.  For now, only part of
the client routines have been interfaced with Julia.  The interface exploits
the power of `ccall` to directly calls the routines of the compiled XPA
library.  The implemented methods are described in what follows, more extensive
XPA documentation can be found
...
There are more simple methods which return only the data part of the answer,
possibly after conversion.  These methods limit the number of answers to be at
most one and throw an error if `xpa_get` returns a non-empty error message.
To retrieve the `data` part of the answer received by an `xpa_get` request as a
...
returns a handle to an XPA permanent connection and which can be used as the
argument of the `xpa` keyword of the `xpa_get` and `xpa_set` methods to speed
up requests.  The handle is automatically closed when the handle is finalized
...
method `xpa_config` provides means to get or set XPA settings.  With a
single argument:
